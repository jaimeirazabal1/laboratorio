...
$cantidad_productos = count($_POST['equipos']['cantidad']);
$productos = array();

for ($i=0; $i < $cantidad_productos; $i++) { 
    $productos[$i]['cantidad'] = $_POST['equipos']['cantidad'][$i];
    $productos[$i]['nombre_del_equipo'] = $_POST['equipos']['nombre_del_equipo'][$i];
    $productos[$i]['rango_de_medida'] = $_POST['equipos']['rango_de_medida'][$i];
    $productos[$i]['resolucion'] = $_POST['equipos']['resolucion'][$i];
    $productos[$i]['marca'] = $_POST['equipos']['marca'][$i];
    $productos[$i]['modelo'] = $_POST['equipos']['modelo'][$i];
    $productos[$i]['serie'] = $_POST['equipos']['serie'][$i];
    $productos[$i]['codigo'] = $_POST['equipos']['codigo'][$i];
    $productos[$i]['equipo_patron'] = $_POST['equipos']['equipo_patron'][$i];
    $productos[$i]['puntos_calibracion'] = $_POST['equipos']['puntos_calibracion'][$i];
}

$sql = sprintf(
        "INSERT INTO equipos  (trackid, data)
        VALUES ('%s', '%s')",
        mysql_real_escape_string($tmpvar['trackid']),
        base64_encode(serialize($productos))
);
//echo $sql;
hesk_dbQuery($sql);
